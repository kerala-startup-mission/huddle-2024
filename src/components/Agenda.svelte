<script>
  ...
  async function loadData(){
    ...
    loading = true;
    let data = await fetch(`https://events.startupmission.in/api/event/huddle-global-2024/agenda/venue`).then(response => response.json());
...
<div class="agenda__tabs space-x-4 text-center text-white mb-5 ">
  {#each Object.entries(agenda_list) as [date, agendas]}
    <button on:click={()=> selectTab(date)} class="px-5 py-3 spl_cursor mb-3 sm:text-base text-xs rounded-md shadow  border {date == active_tab ? 'bg-[#c70000] text-white border-red-600' : 'border-white'} " >
...
<div class="agenda__tabs space-x-4 text-center text-white mb-5 ">
  {#each Object.entries(venues) as [index,venue]}
    <button on:click={()=> selectVenue(venue)} class="px-5 py-3 mb-3  spl_cursor sm:text-base text-xs rounded-md shadow  border {venue == active_venue ? 'bg-[#c70000] text-white border-red-600' : 'border-white'} " >
      <span>{venue}
    </button>
  {/each}
</div>
            ...
            {#each agenda as {name, description, start_time, end_time, category, speakers, venue} }
              ...
              <div class="bg-white alternate-row px-5 py-3">
                <div class="flex flex-col md:flex-row md:space-x-5">
                  <div class=" text-white md:pt-3 flex-shrink-0 min-w-[6rem]">{date_single(start_time)} IST</div>
