<script>
  ...
  <div class="sm:px-6 sm:m-0 m-4">
    <!-- Filter Icon -->
    <div class=" mb-6  items-end justify-end flex flex-col">
      ...
      {#if showSectors}
        <div class=" left-0 z-10 bg-[#f7f3f3] shadow-lg rounded mt-2 md:w-1/3 sm:w-2/3 w-full h-52 overflow-x-hidden overflow-y-scroll">
          {#each sectors as sector}
            <div
              on:click={() => selectSector(sector)}
              class="px-4 py-2 text-black cursor-pointer text-start md:text-base text-sm hover:bg-gray-100"
            >
              {sector}
            </div>
          {/each}
    ...
    <div class="md:grid-cols-2 lg:grid-cols-4 sm:grid-cols-1 grid-cols-1 grid gap-8">
      {#each filteredData as item (item.id)}
        ...
        >
          ...
          <div class="pt-2">
            <div
              class="text-black w-full text-start text-xs flex font-normal"
              style="font-family: Jost, sans-serif;"
            >
            ...
            {#each data as item (item.id)}
              {#if target_val == item.id}
                ...
                >
                  {#if item.attributes.image?.data}
                    <div class="md:w-1/4 w-52 flex my-auto border items-center justify-center">
                      <img
                        src="https://cms.startupmission.in{item.attributes.image.data.attributes.url}"
                        class="w-full object-contain rounded-2xl  my-auto flex items-center justify-center mx-auto"
                      />
                    </div>
                  {/if}
                ...
                <div class=" items-end justify-end flex ">
                    ...
                    class="px-3 py-1 font-bold text-white bg-green-500 rounded-md"
                    >
                    Chat
                    </button>
                </div>
